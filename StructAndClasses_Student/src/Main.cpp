...
{
    if (grade < 0) { throw std::out_of_range("Grade"); }
    if (grade > 12) { throw std::out_of_range("Grade"); }
    this->grade = grade;
}

void Student::Gpa(float gpa)
{
    if (gpa < 0) { throw std::out_of_range("Gpa"); }
    if (gpa > 4.0) { throw std::out_of_range("Gpa"); }
    this->gpa = gpa;
}
