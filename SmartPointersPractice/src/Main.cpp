...
public:
    ResourceManager() : resource_ptr_(new Resource()) {
        std::cout << "ResourceManager constructed\n";
    }

    // No destructor needed!
    ~ResourceManager() {
        std::cout << "ResourceManager destructor called\n";
        // Notice we don't need to delete resourcePtr
    }

private:
    std::unique_ptr<Resource> resource_ptr_;
};
...
void f2(std::shared_ptr<std::string> shared_ptr) {
    // TASK 4: Print the use count property of shared_ptr to the terminal to see how
    // many pointers refer to its resource
...
void f3(std::weak_ptr<std::string> weak_ptr) {
    // TASK 5: Lock the weak pointer by assigning it to a shared pointer. Then, print
    // its content to the terminal.
    //        If the weak ptr can not be locked because the resource it refers to has
    //        expired, print the string "weak pointer is expired" to the terminal.
    // SOLUTION 5:
...
int main() {
    ...
    {
        ResourceManager obj;
        std::cout << "About to destroy ResourceManager\n";
    }
