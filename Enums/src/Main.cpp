...
enum class Jewels {
  ...
  Hearts,
  Spades
}; // Names don't clash as they are different types

enum class Genstones : unsigned long {
  Diamonds,
  Emeralds,
  Opals,
  Rubies,
  Sapphires
}; // The enumerator values can be changed (see parent type unsigned long), but
   // are of type int by default

Suit suit{Suit::Diamonds}; // You must always qualify them using the type name

int main() {
  today = Weekdays::Mon; // you can qualify an enumeration constant with the
  ...
  Weekdays day_after_tomorrow{
      Weekdays::Wed}; // subsequent enumerators start from 10 and are
                      // incremented by 1
  std::cout << "day_after_tomorrow is an integer: " << day_after_tomorrow
            << std::endl;

  int integer = Tues; // Enumerators and variables of an enumeration type will
                      // be implicitly converted to type int when required
